...
async function showLinuxDialog(browsers, wasUninstall) {
  return new Promise((resolve) => {
    ...
    // Check if zenity exists for GUI dialog
    commandExists('zenity').then((hasZenity) => {
      if (!hasZenity) {
      ...
      const args = wasUninstall ?
        ['--info', '--text=' + dialogText, '--title=MAX Video Downloader CoApp', '--width=350'] :
        ['--question', '--text=' + dialogText, '--title=MAX Video Downloader CoApp', '--ok-label=OK', '--cancel-label=Uninstall', '--width=350'];

      const child = spawn('zenity', args, { stdio: 'ignore' });
      ...
      // Fallback to console if command check fails
      const browserList = browsers.map(b => `• ${b.name}`).join('\n');
      const dialogText = wasUninstall ?
        `MAX Video Downloader CoApp removed from ${browsers.length} browser(s):\n\n${browserList}` :
        `MAX Video Downloader CoApp installed for ${browsers.length} browser(s):\n\n${browserList}`;
