...
async function getLinuxDialog(type, title, defaultPath, defaultName) {
    ...
    if (portalResult.cancelled) {
        // User cancelled Portal, throw error (no fallback)
        const error = new Error('Dialog cancelled');
        error.key = type === 'directory' ? 'noDirectorySelected' : 'noSaveLocationSelected';
        throw error;
    }
    // Portal failed, try fallbacks
    ...
    if (!ensureWritable(fallbackPath)) {
        ...
        try {
            ...
            // Ignore tmp dir creation errors
        }
    }
